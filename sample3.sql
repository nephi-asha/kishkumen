...
CREATE TABLE purchase_requests (
    ...
    requested_by_user_id INT NOT NULL, -- Refers to public.users table
    status VARCHAR(50) NOT NULL DEFAULT 'Pending' CHECK (status IN ('Pending', 'Approved', 'Rejected', 'Completed')),
    approved_by_user_id INT, -- Refers to public.users table
...
CREATE TABLE restocks (
    ...
    product_id INT NOT NULL,
    restock_value NUMERIC(10, 2) NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_restocks_product
        FOREIGN KEY (product_id) REFERENCES products(product_id) ON DELETE RESTRICT
);

-- OVERSTOCKS TABLE
CREATE TABLE overstocks (
    overstock_id SERIAL PRIMARY KEY,
    product_id INT NOT NULL,
    quantity_left INT NOT NULL,
    rolled_over BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_overstocks_product
        FOREIGN KEY (product_id) REFERENCES products(product_id) ON DELETE RESTRICT
);
...
CREATE INDEX idx_purchase_items_request_id ON purchase_request_items(request_id);
CREATE INDEX idx_restocks_product_id ON restocks(product_id);
CREATE INDEX idx_overstocks_product_id ON overstocks(product_id);
